...
function createAxiosClient({ baseURL, headers = {}, token } = {}) {
  // Minimal fetch-based client used by the Base44 export for public settings/auth checks.
  const baseHeaders = { ...(headers ?? {}) };
  if (token) baseHeaders.Authorization = `Bearer ${token}`;

  const request = async (method, path) => {
    const res = await fetch(`${baseURL}${path}`, { method, headers: baseHeaders });
...
export const AuthProvider = ({ children }) => {
  ...
  const checkAppState = async () => {
    try {
      ...
      // Local/offline mode: this Base44 export can run without a backend.
      // Avoid calling createAxiosClient (not available in this export) and just let the app render.
      const isLocalMode =
